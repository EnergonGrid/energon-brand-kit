# Energon Brand Guidelines
**Version 1.0**

---

# Project

**Official Name**  
Energon

**Protocol Asset**  
Energon (EON)

**Network**  
Flare Mainnet

**Chain ID**  
14

**Launch**  
December 20, 2025

---

# Official Logo Assets

**Primary Logo**  
Energon-Logo-Primary.png

**Vector Logo**  
Energon-Logo.svg

**Application Icon**  
Energon-Logo-512.png

**Favicon**  
Energon-Favicon.ico

**Apple Touch Icon**  
Energon-Apple-Touch.png

---

# Logo Usage

Use the **Primary Logo** whenever possible.

The **SVG** version should be used for websites, documentation, presentations, and print.

The **PNG** versions should be used for wallets, block explorers, social media, applications, and any environment that does not support SVG.

Do **not**:

- Modify the logo colors
- Stretch or distort the logo
- Rotate the logo
- Change the typography
- Add effects or overlays
- Combine the logo with other graphics in a way that implies official endorsement

Maintain adequate clear space around the logo whenever possible.

---

# Official Resources

## Main Website

https://energon.app

## Guardian Chronicle

https://guardian.energon.app

## Wallet Setup

https://wallet.energon.app

## Documentation

https://docs.energon.app

## Whitepaper

https://docs.energon.app/docs/energon-whitepaper.pdf

## Energon Management Protocol (EMP)

https://docs.energon.app/docs/energon-emp.pdf

## Why One Cube

https://why.energon.app

## Mint

https://mint.energon.app

## Dashboard

https://dashboard.energon.app

## Observer

https://observer.energon.app

## GitHub

https://github.com/EnergonGrid

## X (Twitter)

https://x.com/EnergonProtocol

## Discord

https://discord.gg/9fP8sr55Zw

## Email

EnergonGrid@gmail.com

---

# Token Information

**Name**  
Energon

**Symbol**  
EON

**Standard**  
ERC-20

**Network**  
Flare Mainnet

**Chain ID**  
14

**Decimals**  
18

**Maximum Supply**  
30,000,000 EON

**Contract Address**

```
0x9458Cbb2e7DafFE6b3cf4d6F2AC75f2d2e0F7d79
```

---

# EnergonCube

**Standard**  
ERC-721

**Maximum Supply**  
1,000,000 EnergonCubes

**Contract Address**

```
0x30e1076bDf2B123B54486C2721125388af2d2061
```

---

# Official Motto

> **One Wallet • One Cube • One Guardian**

---

# Brand Principles

The Energon brand represents:

- Permanence
- Transparency
- Determinism
- Observable protocol state
- Verifiable on-chain identity

Avoid messaging that implies:

- Guaranteed profits
- Investment returns
- Yield generation
- Governance control over immutable protocol rules

Energon is an immutable blockchain protocol whose behavior is defined by deployed smart contracts and observable on-chain state.

---

© Energon Protocol